...
async function addSoal(token, payload) {
  try {
    ...
    console.log(`Successfully added soal: ${payload.category} - ${payload.question.substring(0, 30).replace(/\n/g, ' ')}...`);
  } catch (err) {
...
async function run() {
  ...
  for (const mod of modulesToProcess) {
    ...
    const filePath = `../../data/pembahasan/hasil_pembahasan_modul_${mod.id}/pembahasan_modul_${mod.id}.json`;
    ...
    for (const item of data) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (mod.category === 'TKP') {
          ...
          options.push({ label: labels[i], text: parsed.text, score: parsed.score || 1 });
        } else {
          options.push({ label: labels[i], text: optText });
          if (item.jawaban_benar && item.pilihan[i] && optText.trim() === item.jawaban_benar.trim()) {
      ...
      if (mod.category !== 'TKP') {
        if (!correctAnswer && item.jawaban_benar) {
          for (let i = 0; i < item.pilihan.length; i++) {
            if (item.jawaban_benar.includes(item.pilihan[i].trim()) || item.pilihan[i].includes(item.jawaban_benar.trim())) {
              correctAnswer = labels[i];
              break;
            }
          }
